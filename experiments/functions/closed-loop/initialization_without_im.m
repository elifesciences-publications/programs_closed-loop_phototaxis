...
%% Comments
% Initialization of the stereovisual and spatio_temporal experiment.  Don't record
% the images
% Inputs -----
...
while timeStamprec < ini_rec
    ...
    if timeStamprec < ini_rec && n == 1
        ...
        if i > 1 && abs(angle_ini(nb_seq_ini,i)-angle_ini(nb_seq_ini,i-1)) > 140
    ...
    elseif timeStamprec >= ini_rec
        if framerate_ini(end,2) == 0
            framerate_ini(end,2) = timeStamprec; %ending time of a sequence
            framerate_ini(end,3) = i/(framerate_ini(end,2)-framerate_ini(end,1));
            framerate_ini(end,4) = i;
        end
    end
end
