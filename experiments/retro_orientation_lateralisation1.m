%% Comments
% Spatio-temporal experiment or lateralisation experiment
...
%luminosity_linear, lateralisation_f, center_f, crop_image,
%experiment_lateralisation_without_im, experiment_lateralisation_with_im,
%image_processing
...
% ----- initalization -----
ini_tot = 0*60; %total lenght of the initialisation in seconds
ini_rec = 0*60; %duration for recording baseline
textureOpacityini = 1;

% ----- experiment -----
percPm = 1; %percentage max of the luminosity during the experiment
lum_lin = luminosity_linear(percPm);
recording_time_sec = 2*60; %total lenght of the experiement in seconds
time_adapt = 0; %adaptation time after OMR
ang_t = 40; %angle for the black eye convergence zone in degree
width = 15; %width of the black line under the fish, in pixels
...
%% Experiment ------------------------------------------------------
if im == 0 % no image recording
    [time_lost,angle,angle_cum,angle_filtered,centroids,head,...
        framerate_rec,error_head] = experiment_lateralisation_without_im(xin,yin,ang_thresh,...
...
directory='F:\Projects\Julie\data\lateralisation';
...
D.experiment.angleFiltered = angle_filtered;
D.experiment.coordinates = centroids;
D.experiment.head = head;
