%% Comments
% Stereovisual experiment with an exponential profile
...
%luminosity_exponentielle, center_f, crop_image,
%experiment_exp_without_im, experiment_exp_with_im, image_processing,
...
% ----- initalisation -----
ini_tot = 10*60; %total lenght of the initialisation in seconds
ini_rec = 8*60; %duration for recording baseline
textureOpacityini = 1;

% ----- experiment -----
percPm = 0.6; %percentage max of the luminosity during the experiment
lum_exp = luminosity_exponentielle(percPm);
...
if im == 0 % no image recording
    [time_lost,angle,angle_cum,angle_illum,luminosity,centroids,...
        framerate_rec,f_lost,error_head] = experiment_exp_without_im(xin,yin,ang_thresh,...
        recording_time_sec,time_adapt,lum_exp,...
        vid,window,mask_white,srcRect,screenXpixels,screenYpixels,ifi,black,vbl,...
        waitframes,imT,bwthresh,small);
elseif im == 1 %image recording
    [time_lost,angle,angle_cum,angle_illum,luminosity,centroids,...
        framerate_rec,f_lost,error_head,fish_im] = experiment_exp_with_im(xin,yin,ang_thresh,...
        recording_time_sec,time_adapt,lum_exp,...
        vid,window,mask_white,srcRect,screenXpixels,screenYpixels,ifi,black,vbl,...
        waitframes,imT,bwthresh,small);
end
...
directory='F:\Projects\Julie\data\utilisables_lum_exp';
